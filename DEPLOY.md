...
```bash
GSC_KEY=~/.ssh/gsc-....json npm run gsc              # 최근 28일 실적
GSC_KEY=... npm run gsc -- --index                   # 색인 상태까지
GSC_KEY=... npm run gsc -- --index --log ~/gsc.tsv   # 한 줄씩 쌓아 추이를 본다
```

주 1회 호스트 crontab에서 돈다(월요일 07:10). `~/gsc.tsv`에 한 줄씩,
전체 리포트는 `~/gsc.log`에 쌓인다. node 경로가 nvm이라 크론 줄에 절대경로로
박혀 있다 — **버전을 올리면 그 줄도 고쳐야 한다.**

`--dump`로 주소별 결과를 남겨 두면 지난 회차와 무엇이 달라졌는지 짚을 수
있다. 표의 숫자만으로는 어느 주소가 움직였는지가 안 남는다.

읽기 전용 서비스 계정을 만들어 서치콘솔 속성에 `제한된 사용자`로 넣고,
그 JSON 경로를 넘긴다. **열쇠는 저장소에 두지 않는다.** 로그 파일도
저장소 밖에 둔다.

색인이 안 된 페이지는 두 가지가 섞여 있고, 할 일이 서로 다르다.

| | |
| --- | --- |
| 크롤하고도 색인 안 함 | 페이지 쪽에 이유가 있다. 여기부터 본다 |
| 아직 안 가져감 | 순서가 안 온 것이다. 기다린다 |

서치콘솔 화면은 둘을 `Discovered - currently not indexed` 한 줄로 묶어
보여 줘서 고칠 것이 있는 것처럼 읽힌다. 이 스크립트는 갈라서 찍는다.

`--index`는 sitemap의 주소를 하나씩 묻는다. 한 건에 7초, 하루 2000건
한도다 — 주소가 900개면 20분이다. 주 1회로 잡는다.
