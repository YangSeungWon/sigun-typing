 ...
 *
 * 기준은 순위표와 같다 — 많이 끝낸 쪽이 먼저, 같으면 빠른 쪽이 먼저.
 * 여기서만 다른 기준을 쓰면 "상위 8%"를 보고 등록했는데 다른 자리에 가 있게 된다.
 */
export async function GET(request: Request) {
  const url = new URL(request.url);
  const courseId = url.searchParams.get("course") ?? "";
  const mode = url.searchParams.get("mode") ?? "";
  const completed = Number(url.searchParams.get("completed"));
  const elapsedMs = Number(url.searchParams.get("elapsed"));
  ...
  const [{ better, total }, around] = await Promise.all([
    repo.standing(courseId, mode, SCORING_VERSION, course.version, key),
    // 바로 위·아래 두 줄씩. 상위 10명보다 이쪽이 따라잡을 마음을 만든다.
    repo.neighbors(courseId, mode, SCORING_VERSION, course.version, key, 2),
  ]);
