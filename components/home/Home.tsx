...
export function Home({ seed, geo }: { seed: HomeSeed; geo: CourseGeo | null }) {
   ...
   *
   * 위는 늘 **곳 이름**, 아래는 늘 `전체`다.
   *
   * 아래를 `대한민국`이라고 불렀었다. 전국 코스의 짧은 이름이 `전국`이라
   * 245 쪽도 `전국`이면 같은 낱말이 다른 분모를 달고 위아래로 붙기 때문인데,
   * 피한 자리에 같은 뜻의 낱말을 놓은 것이라 아무것도 안 풀렸다 — 전국과
   * 대한민국은 같은 말이고, 게다가 13은 63 안에 들어 있다(245 = 시도 17 +
   * 시군구 228). 같은 뜻의 두 낱말이 포개진 두 범위를 가리키고 있었다.
   *
   * (그때 `대한민국`을 고른 다른 이유는 아래 정복도 카드와 맞추는 것이었는데,
   * 그 카드는 지도 옆 목록이 생기면서 지웠다.)
   *
   * 낱말이 아니라 **종류**를 다르게 둔다. 위는 부산·경기·전국처럼 고른 곳의
   * 이름이고 아래는 그것을 담는 그릇이다. `전체`는 곳 이름이 아니므로 무엇과도
   * 겹치지 않고, 전국이 전체 안에 든다는 것은 읽어서 안다.
   */
  const nationwide = {
    label: "전체",
