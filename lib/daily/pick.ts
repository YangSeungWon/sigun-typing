 ...
 * 그 일차가 며칠인가. `8월 25일`.
 *
 * 화면에는 `7일차`라고 적고 있었다. 이 값이 세는 것은 서비스가 시작한 지
 * 며칠인가인데, 읽는 쪽에서는 **내가 이레째**로 먼저 읽힌다. 하필 바로
 * 아래에 이어 온 날을 세는 불꽃이 있어서 두 숫자가 같은 말을 하는 것처럼
 * 보였다.
 *
 * 날짜는 그렇게 안 읽힌다. 게다가 받은 사람이 어제 것인지 오늘 것인지
 * 아는 값이라, 단톡방에 붙는 격자에도 이쪽이 맞다.
 *
