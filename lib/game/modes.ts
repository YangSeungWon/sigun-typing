...
export const MODES: Record<ModeId, ModeConfig> = {
  ...
  map: {
    ...
    allowHint: true,
    /*
     * 힌트에 시간을 물리지 않는다. **횟수로 센다.**
     *
     * 한때 한 번에 30초를 얹었다. 겨루는 판과 배우는 판을 가르려는 값이었는데,
     * 그 무게가 코스 길이에 따라 널뛰었다 — 제주 두 곳(5~10초)에서는 한 번에
     * 판이 끝장나고 전국 229곳(5~10분)에서는 티도 안 났다. 같은 `힌트 한 번`을
     * 코스에 따라 다르게 벌하고 있었던 것이다.
     *
     * 순위는 힌트 적은 순, 그다음이 시간이다. 코스 길이와 무관하게 일정하고,
     * 화면의 시간이 벽시계와 같아진다.
     */
  },
