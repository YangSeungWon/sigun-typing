...
const COURSES: CourseSummary[] = [
  ...
  { id: "gyeonggi", name: "경기도 31 시군", shortName: "경기", version: 1, total: 31, sido: "41" },
];
...
const TODAY = "gyeonggi";

describe("이어하기가 가리킬 곳", () => {
  it("처음이면 입문 코스에서 시작한다", () => {
    const t = pickResumeTarget(COURSES, new Map(), null, TODAY);
  ...
  it("마지막 기록이 없어도 해 본 코스가 있으면 이어한다", () => {
    const t = pickResumeTarget(COURSES, mastery({ gyeonggi: 30 }), null, TODAY);
    expect(t).toMatchObject({ kind: "resume", courseId: "gyeonggi", known: 30 });
  });

  it("전부 정복했으면 오늘의 도전으로 보낸다", () => {
    const t = pickResumeTarget(
      COURSES,
    ...
    expect(t).toMatchObject({ kind: "resume", courseId: TODAY });
  });

  it("해 본 적 없는 코스로는 이어하지 않는다", () => {
    // 남은 곳이 31로 가장 많지만 손댄 적이 없다.
    const t = pickResumeTarget(COURSES, mastery({ seoul: 18 }), null, TODAY);
    expect(t.courseId).toBe("seoul");
  });
});
